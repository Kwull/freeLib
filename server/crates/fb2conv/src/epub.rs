...
pub(crate) fn identifier(fb2_id: &str, content_hash: u128) -> String {
    let id = fb2_id.trim();
    if is_uuid(id) {
        return format!("urn:uuid:{}", id.trim_start_matches('{').trim_end_matches('}').to_ascii_lowercase());
    }
    let h = if id.is_empty() { content_hash } else { hash128(id.as_bytes()) };
...
fn toc_tree(toc: &[TocEntry]) -> (Vec<TreeNode>, Vec<usize>) {
    ...
    for (i, e) in toc.iter().enumerate() {
        ...
        nodes.push(TreeNode { entry: i, children: Vec::new() });
        match stack.last() {
...
fn nav_doc(p: &Package) -> String {
    ...
    if let Some(c) = p.cover_file {
        body.push_str(&format!("  <li><a epub:type=\"cover\" href=\"{}\">", esc(&p.files[c].name)));
    ...
    if let Some(f) = p.files.get(p.body_start) {
        body.push_str(&format!("  <li><a epub:type=\"bodymatter\" href=\"{}\">", esc(&f.name)));
...
fn ncx_points(p: &Package, nodes: &[TreeNode], list: &[usize], order: &mut usize, depth: usize, max_depth: &mut usize, out: &mut String) {
    *max_depth = (*max_depth).max(depth);
    for &n in list {
        let e = &p.toc[nodes[n].entry];
        *order += 1;
        out.push_str(&format!("<navPoint id=\"np{0}\" playOrder=\"{0}\"><navLabel><text>", *order));
        esc_text(if e.title.is_empty() { "-" } else { &e.title }, out);
        out.push_str(&format!("</text></navLabel><content src=\"{}\"/>\n", esc(&href(p, e))));
        ncx_points(p, nodes, &nodes[n].children, order, depth + 1, max_depth, out);
...
fn ncx(p: &Package) -> String {
    ...
    s.push_str(&format!("<meta name=\"dtb:uid\" content=\"{}\"/>\n", esc(&p.identifier)));
    s.push_str(&format!("<meta name=\"dtb:depth\" content=\"{}\"/>\n", depth.saturating_sub(1).max(1)));
    s.push_str("<meta name=\"dtb:totalPageCount\" content=\"0\"/>\n<meta name=\"dtb:maxPageNumber\" content=\"0\"/>\n</head>\n<docTitle><text>");
    esc_text(if p.info.title.is_empty() { "-" } else { &p.info.title }, &mut s);
...
fn opf(p: &Package) -> String {
    ...
    m.push_str(&format!("<dc:identifier id=\"bookid\">{}</dc:identifier>\n", esc(&p.identifier)));
    m.push_str(&format!("<dc:title id=\"title\">{}</dc:title>\n", esc(if i.title.is_empty() { "-" } else { &i.title })));
    m.push_str(&format!("<dc:language>{}</dc:language>\n", esc(p.lang)));
    for (n, a) in i.authors.iter().enumerate() {
        let id = format!("creator{}", n + 1);
        m.push_str(&format!("<dc:creator id=\"{id}\">{}</dc:creator>\n", esc(&a.natural_name())));
        m.push_str(&format!("<meta refines=\"#{id}\" property=\"role\" scheme=\"marc:relators\">aut</meta>\n"));
        m.push_str(&format!("<meta refines=\"#{id}\" property=\"file-as\">{}</meta>\n", esc(&a.file_as())));
    }
    for (n, t) in i.translators.iter().enumerate() {
        let id = format!("translator{}", n + 1);
        m.push_str(&format!("<dc:contributor id=\"{id}\">{}</dc:contributor>\n", esc(&t.natural_name())));
        m.push_str(&format!("<meta refines=\"#{id}\" property=\"role\" scheme=\"marc:relators\">trl</meta>\n"));
    }
    if let Some(a) = &i.annotation {
        // plain text description
        let (root, _) = crate::dom::parse(&format!("<d>{}</d>", a.replace("<br>", "<br/>").replace("</p>", "</p>\n")), None);
        let txt = root.text();
        let txt = txt.lines().map(|l| l.split_whitespace().collect::<Vec<_>>().join(" ")).filter(|l| !l.is_empty()).collect::<Vec<_>>().join("\n");
        if !txt.is_empty() {
    ...
    if !i.publisher.is_empty() {
        m.push_str(&format!("<dc:publisher>{}</dc:publisher>\n", esc(&i.publisher)));
    }
    let year = [&i.date, &i.year].iter().map(|d| d.trim().chars().take(4).collect::<String>()).find(|y| y.len() == 4 && y.chars().all(|c| c.is_ascii_digit()));
    if let Some(y) = year {
        m.push_str(&format!("<dc:date>{y}</dc:date>\n"));
    }
    if !i.isbn.is_empty() {
        m.push_str(&format!("<dc:identifier id=\"isbn\">urn:isbn:{}</dc:identifier>\n", esc(&i.isbn.replace([' ', '-'], ""))));
    }
    m.push_str(&format!("<meta property=\"dcterms:modified\">{}</meta>\n", now_utc()));
    if let Some(s) = &i.series {
        m.push_str(&format!("<meta property=\"belongs-to-collection\" id=\"series\">{}</meta>\n", esc(s)));
        m.push_str("<meta refines=\"#series\" property=\"collection-type\">series</meta>\n");
        if let Some(n) = i.serno {
            m.push_str(&format!("<meta refines=\"#series\" property=\"group-position\">{n}</meta>\n"));
        }
        m.push_str(&format!("<meta name=\"calibre:series\" content=\"{}\"/>\n", esc(s)));
        if let Some(n) = i.serno {
            m.push_str(&format!("<meta name=\"calibre:series_index\" content=\"{n}\"/>\n"));
        }
    }
    ...
    for (n, f) in p.files.iter().enumerate() {
        ...
        man.push_str(&format!("<item id=\"x{n}\" href=\"{}\" media-type=\"application/xhtml+xml\"{props}/>\n", esc(&f.name)));
    }
    for (n, r) in p.resources.iter().enumerate() {
        if r.cover_image {
            ...
            man.push_str(&format!("<item id=\"r{n}\" href=\"{}\" media-type=\"{}\"/>\n", esc(&r.href), r.media_type));
        }
    }
    ...
    if let Some(c) = p.cover_file {
        guide.push_str(&format!("<reference type=\"cover\" title=\"{}\" href=\"{}\"/>\n", esc(p.labels.cover), esc(&p.files[c].name)));
    }
    if p.spine.contains(&SpineItem::Nav) {
        guide.push_str(&format!("<reference type=\"toc\" title=\"{}\" href=\"nav.xhtml\"/>\n", esc(p.labels.contents)));
    }
    if let Some(f) = p.files.get(p.body_start) {
        guide.push_str(&format!("<reference type=\"text\" title=\"{}\" href=\"{}\"/>\n", esc(p.labels.start), esc(&f.name)));
    }
...
pub(crate) fn deflated() -> SimpleFileOptions {
    SimpleFileOptions::default().compression_method(CompressionMethod::Deflated).compression_level(Some(6))
}

pub(crate) fn write(p: &Package) -> Result<Vec<u8>> {
    let size_hint = p.files.iter().map(|f| f.body.len()).sum::<usize>() / 3 + p.resources.iter().map(|r| r.data.len()).sum::<usize>();
    ...
    zw.start_file("META-INF/container.xml", deflated()).map_err(zip_err)?;
    zw.write_all(CONTAINER.as_bytes())?;
    if p.embed_fonts {
        zw.start_file("META-INF/com.apple.ibooks.display-options.xml", deflated()).map_err(zip_err)?;
        zw.write_all(APPLE_OPTIONS.as_bytes())?;
    }
    zw.start_file("OEBPS/content.opf", deflated()).map_err(zip_err)?;
    zw.write_all(opf(p).as_bytes())?;
    zw.start_file("OEBPS/toc.ncx", deflated()).map_err(zip_err)?;
    zw.write_all(ncx(p).as_bytes())?;
    zw.start_file("OEBPS/nav.xhtml", deflated()).map_err(zip_err)?;
    zw.write_all(nav_doc(p).as_bytes())?;
    zw.start_file("OEBPS/css/main.css", deflated()).map_err(zip_err)?;
    zw.write_all(p.css.as_bytes())?;
    for f in &p.files {
        let title = if f.title.is_empty() { &p.info.title } else { &f.title };
        let doc = xhtml_doc(p.lang, title, f.body_class, &f.body);
        zw.start_file(format!("OEBPS/{}", f.name), deflated()).map_err(zip_err)?;
        zw.write_all(doc.as_bytes())?;
    }
    let mut fonts = if p.resources.iter().any(|r| r.packed) { zip::ZipArchive::new(Cursor::new(p.font_archive)).ok() } else { None };
    for r in &p.resources {
        if r.packed
            && let Some(f) = fonts.as_mut().and_then(|a| a.by_name(&format!("OEBPS/{}", r.href)).ok())
        {
            zw.raw_copy_file(f).map_err(zip_err)?;
            continue;
        }
        let opts = if r.media_type.starts_with("font/") || r.media_type == "image/svg+xml" || r.media_type == "image/bmp" { deflated() } else { stored() };
        zw.start_file(format!("OEBPS/{}", r.href), opts).map_err(zip_err)?;
        zw.write_all(&r.data)?;
    }
