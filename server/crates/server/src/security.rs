...
pub fn check_csrf(req: &Request<Body>) -> Result<(), ApiError> {
    ...
    let h = req.headers();
    let site = h.get("sec-fetch-site").and_then(|v| v.to_str().ok()).unwrap_or("");
        ...
        && origin != "null"
    {
        ...
        let ok = hosts
            ...
            .any(|host| host.split(',').next().is_some_and(|x| x.trim().eq_ignore_ascii_case(o)));
        if !ok {
    ...
    if has_body {
        let ct = h.get(header::CONTENT_TYPE).and_then(|v| v.to_str().ok()).unwrap_or("");
        if !ct.trim_start().to_ascii_lowercase().starts_with("application/json") {
...
pub async fn csrf_layer(req: Request<Body>, next: Next) -> Response {
    ...
    let h = r.headers_mut();
    h.entry("x-content-type-options").or_insert(header::HeaderValue::from_static("nosniff"));
    r
}
...
pub async fn security_headers(req: Request<Body>, next: Next) -> Response {
    ...
    let h = r.headers_mut();
    h.entry("x-content-type-options").or_insert(header::HeaderValue::from_static("nosniff"));
    h.entry("referrer-policy").or_insert(header::HeaderValue::from_static("same-origin"));
    h.entry("x-frame-options").or_insert(header::HeaderValue::from_static("SAMEORIGIN"));
    r
}
