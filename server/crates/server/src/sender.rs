...
pub async fn start(st: &AppState, user: &User, req: SendRequest) -> ApiResult<Job> {
    ...
    if req.books.len() > MAX_BOOKS {
        return Err(ApiError::bad_request(format!("at most {MAX_BOOKS} books per request")));
    }
    let dev = &req.device;
    if matches!(dev.format.as_str(), "azw3" | "mobi" | "pdf") && st.calibre.is_none() {
        return Err(ApiError::unsupported(format!("{} needs Calibre, which is not installed", dev.format)));
    }
    let target = req.target.clone().filter(|t| !t.trim().is_empty()).or(dev.target.clone());
    let smtp = match dev.kind.as_str() {
        "email" => {
            ...
            if smtp.host.trim().is_empty() {
                return Err(ApiError::bad_request("SMTP server is not configured (Settings → Mail)"));
            }
            Some(smtp)
        }
        "folder" => {
    ...
    let (kind, title) = match dev.kind.as_str() {
        "email" => ("send", format!("Send to {} · {n} book{}", dev.name, if n == 1 { "" } else { "s" })),
        "folder" => ("export", format!("Export to {} · {n} book{}", dev.name, if n == 1 { "" } else { "s" })),
        _ => ("download", format!("Download for {} · {n} book{}", dev.name, if n == 1 { "" } else { "s" })),
    };
...
async fn run(
    ...
    let details: Vec<BookDetail> =
        tokio::task::spawn_blocking(move || -> ApiResult<Vec<BookDetail>> { Ok(ids.iter().filter_map(|id| cat.book(*id).ok().flatten()).collect()) })
            .await??;
    if details.is_empty() {
        return Err(ApiError::not_found("books not found"));
    }
    let lib_dir = crate::api::books::lib_dir(st, req.library).await?;
    let template = req.file_name.clone().filter(|t| !t.trim().is_empty()).unwrap_or(dev.file_name.clone());
    ...
    for (i, unit) in units.iter().enumerate() {
        ...
        st.jobs.running(job_id, i as f64 / total as f64, &first.book.title);
        let res = produce_unit(st, req.library, &lib_dir, unit, &dev.format, &opts).await;
        let (data, ext) = match res {
            Ok(x) => x,
            Err(e) => {
                errors += 1;
                st.jobs.log(job_id, &format!("{}: {}", first.book.title, e.message));
                continue;
            }
        };
        ...
        if unit.books.len() > 1 {
            ...
            book.title = book.series.as_ref().map(|s| s.name.clone()).unwrap_or(book.title);
            book.serno = None;
        }
        let base = st.conv.file_name(&template, &output::name_fields(&book), opts.transliterate);
        match dev.kind.as_str() {
            "email" => {
                let smtp = smtp.as_ref().ok_or_else(|| ApiError::internal("smtp missing"))?;
                ...
                match crate::mail::send(smtp, &to, &book.title, "Sent by freeLib", Some((&name, mime, data))).await {
            ...
            "folder" => {
                ...
                let path = join_safe(&dir, &format!("{base}.{ext}")).ok_or_else(|| ApiError::bad_request("invalid file name"))?;
            ...
            _ => {
                let rel = if multi_file { format!("{base}.{ext}") } else { format!("{}.{ext}", base.replace('/', " - ")) };
    ...
    let summary = if errors > 0 { format!("{} of {total} done, {errors} failed", total - errors) } else { format!("{total} done") };
    if dev.kind == "download" {
        let file = if produced.len() == 1 {
            let (name, p) = produced.remove(0);
            let ext = name.rsplit('.').next().unwrap_or("").to_string();
            JobFile { path: p, mime: mime_for_ext(&ext).into(), name }
        } else {
            ...
            JobFile { path: zip_path, mime: "application/zip".into(), name: format!("freeLib - {} books.zip", produced.len()) }
        };
...
fn make_units(details: Vec<BookDetail>, format: &str, opts: &ConvertOptions) -> Vec<Unit> {
    if !opts.join_series || format == "original" {
        return details.into_iter().map(|b| Unit { books: vec![b] }).collect();
    }
...
async fn produce_unit(
    ...
    if unit.books.len() == 1 {
        ...
        return Ok((p.into_bytes().await?, output::file_ext(format, &first.book.ext)));
    }
    ...
    for b in &unit.books {
        ...
        sources.push(tokio::task::spawn_blocking(move || crate::bookio::read_original(&dir, &d)).await??);
    }
    ...
    match format {
        ...
        f => Ok((output::calibre_run(st, &epub, "epub", f).await?, f.to_string())),
    }
}
...
fn write_zip(path: &std::path::Path, items: &[(String, PathBuf)]) -> ApiResult<()> {
    ...
    for (name, p) in items {
        ...
        let text = [".fb2", ".txt", ".html", ".htm", ".rtf"].iter().any(|e| n.to_lowercase().ends_with(e));
        z.start_file(n, if text { deflated } else { stored }).map_err(|e| ApiError::internal(format!("zip: {e}")))?;
        let data = std::fs::read(p)?;
        z.write_all(&data)?;
    }
    z.finish().map_err(|e| ApiError::internal(format!("zip: {e}")))?.flush()?;
    Ok(())
}
